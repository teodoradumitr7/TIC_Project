...
router.post("/rentals/:id",checkAuthorization,async (req, res) => {
  ...
  let rental = {
    vin: req.body.vin,
      ...
      price:req.body.price*daysBetweenDates(req.body.dateEnd,req.body.dateStart)
  };
  ...
  if(rental.dateEnd.includes('/'))
{
  ...
  let ok=1
  const allRentalsForVin=await db.collection("rentals").doc(rental.vin).collection("rented").get();
...
router.put("/rentals/:id", checkAuthorization,async (req, res) => {
  ...
  let rental = {
    ...
    dateEnd:req.body.dateEnd,
    days:daysBetweenDates(req.body.dateEnd,req.body.dateStart),
    price:daysBetweenDates(req.body.dateEnd,req.body.dateStart)*req.body.price
  };
...
router.get("/rentals/:email", async (req, res) => {
  let rentals = [];
  const result =  await db.collection("users").doc(req.params.email).collection("rented").get();
  result.forEach((doc) => {
    let rental = {
      id: doc.id,
      days: doc.data().days,
